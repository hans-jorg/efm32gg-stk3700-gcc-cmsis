...
uint32_t
ClockSetHFCLKDivisor(uint32_t div) {
 ...
 */
uint32_t
ClockSetHFPrescalers(uint32_t corediv, uint32_t perdiv) {
uint32_t c,p;
...
/**
 * @brief       Get HFCORECLOCK Frequency
 *
 * @note
 *
 */
...
uint32_t
ClockGetHFCoreClockFreq(void) {

    uint32_t freq = SystemHFClockGet();

    uint32_t div = (CMU->HFCORECLKDIV& _CMU_HFCORECLKDIV_HFCORECLKDIV_MASK)>>_CMU_HFCORECLKDIV_HFCORECLKDIV_SHIFT;
    // divisor is encoded as a power of 2 exponent
    return freq>>div;
}
...
uint32_t
ClockGetHFPerClockFreq(void) {

    if( (CMU->HFPERCLKDIV&CMU_HFPERCLKDIV_HFPERCLKEN) == 0 )
        return 0;   // Not enabled. Return 0

    uint32_t freq = SystemHFClockGet();

    uint32_t div = (CMU->HFPERCLKDIV&_CMU_HFPERCLKDIV_HFPERCLKDIV_MASK)>>_CMU_HFPERCLKDIV_HFPERCLKDIV_SHIFT;
    // divisor is encoded as a power of 2 exponent
    return freq>>div;
}
